...
async function fetchHistoricalDataInBackground(universe, dailyFromDateStr, dailyToDateStr, min15FromDateStr) {
  ...
  for (const item of universe) {
    ...
    try {
      ...
      // Silently ignore or minimal log to avoid spamming 2000 errors if API limit hit
    }
    ...
    const initialSignal = strategy.calculate(state);
    if (initialSignal !== 'NONE' && state.signal === 'NONE') {
      state.signal = initialSignal;
